...
@section('content')
@include('marketplace::_breadcrumbs', ['admin' => true, 'items' => [['label' => trans('marketplace::admin.tags.title')]]])
            ...
            <tbody>
                @forelse($tags as $tag)
                    <tr>
                        ...
                        <td>@if($tag->category)<span class="badge bg-primary-subtle text-primary-emphasis"><i class="bi bi-grid me-1" aria-hidden="true"></i>{{ $tag->category->name }}</span>@else<span class="badge bg-info-subtle text-info-emphasis"><i class="bi bi-globe2 me-1" aria-hidden="true"></i>@lang('marketplace::admin.tags.general')</span>@endif</td>
                ...
                @empty
                    <tr><td colspan="6" class="text-center py-5"><i class="bi bi-tags fs-1 text-muted d-block mb-2" aria-hidden="true"></i><strong class="d-block">@lang('marketplace::admin.tags.empty')</strong><span class="text-muted">@lang('marketplace::admin.tags.empty_help')</span></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

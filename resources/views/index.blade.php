...
<div class="container content">
    ...
    <div class="row g-4">
            ...
            <div class="marketplace-sidebar-wrap">
                <div class="card marketplace-sidebar">
                    ...
                    <div class="list-group list-group-flush">
                        <a href="{{ route('marketplace.index', ['sort' => $sort]) }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center gap-2 @if(! isset($category) && ! $mine) active @endif"><span class="text-truncate"><i class="bi bi-collection marketplace-category-icon me-2"></i>@lang('marketplace::messages.all_categories')</span><span class="badge rounded-pill {{ ! isset($category) && ! $mine ? 'bg-light text-dark' : 'bg-secondary' }}">{{ $categories->sum('resources_count') }}</span></a>
        ...
        <main class="col-lg-9">
            ...
            <div class="d-flex justify-content-between align-items-end gap-3 mb-3"><div><h2 class="h4 mb-1">{{ $mine ? trans('marketplace::messages.my_resources') : ($category->name ?? trans('marketplace::messages.all_resources')) }}</h2><p class="text-muted mb-0">@choice('marketplace::messages.results', $resources->total(), ['count' => $resources->total()])</p></div></div>
